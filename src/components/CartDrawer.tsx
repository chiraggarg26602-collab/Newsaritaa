...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const handleApplyPromo = (e: React.FormEvent) => {
    e.preventDefault();
    setPromoError('');
    if (promoInput.trim().toUpperCase() === 'NAYAAB10') {
      setAppliedPromo({ code: 'NAYAAB10', pct: 10 });
      setPromoInput('');
    } else {
      setPromoError('Invalid coupon code. Try NAYAAB10');
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-end animate-fadeIn">
      <div className="bg-[#FAF7F2] w-full max-w-md h-full shadow-2xl flex flex-col justify-between border-l border-[#EADFCF]">
        ...
        <div className="p-5 bg-white border-b border-[#EADFCF] flex items-center justify-between">
          ...
          <button 
            onClick={onClose}
            className="p-1.5 rounded-full text-[#2C2A29] hover:bg-[#FAF7F2] transition"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 bg-[#F5F0EB] border-b border-[#EADFCF] text-xs">
          {subtotal >= freeShippingThreshold ? (
            ...
            <div>
              <div className="flex justify-between text-[#1A1918] font-medium mb-1.5">
                <span>Add ₹{amountNeededForFreeShipping.toLocaleString('en-IN')} more for FREE Express Delivery</span>
                <span>{Math.round(progressPct)}%</span>
              </div>
              <div className="w-full h-2 bg-[#EADFCF] rounded-full overflow-hidden">
                <div 
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 divide-y divide-[#EADFCF]">
          {cartItems.length === 0 ? (
            <div className="py-16 text-center space-y-3">
              <ShoppingBag className="w-12 h-12 text-[#D4A359] mx-auto opacity-50" />
              <h4 className="font-serif-display text-xl font-bold text-[#1A1918]">
                Your Shopping Bag is Empty
              </h4>
              <p className="text-xs text-[#7A6B5D] max-w-xs mx-auto">
                Explore our handblock printed Bagru, Sanganeri, and Ajrakh collections to add artisanal handcrafted pieces.
              </p>
              <button
                onClick={onClose}
                className="mt-4 px-6 py-2.5 bg-[#C85A32] text-white text-xs font-semibold rounded-md hover:bg-[#B04B26] transition"
              >
                Start Shopping Now
              </button>
            </div>
                  ...
                  <div>
                    <div className="flex items-start justify-between">
                      ...
                      <button
                        ...
                        className="text-[#9E8E81] hover:text-[#C85A32] p-1 transition"
                        title="Remove item"
                      >
                    ...
                    <p className="text-[11px] text-[#7A6B5D]">
                      Craft: {item.product.craftType} • Size: <strong className="text-[#1A1918]">{item.selectedSize}</strong>
                    </p>
                  </div>

                  <div className="flex items-center justify-between mt-2">
                    {/* Quantity Controls */}
                    <div className="flex items-center border border-[#EADFCF] rounded bg-white text-xs">
                      <button
                        onClick={() => onUpdateQuantity(item.product.id, item.selectedSize, item.quantity - 1)}
                        className="px-2 py-0.5 text-[#2C2A29] hover:bg-[#F5F0EB]"
                      >
                        -
                      </button>
                      <span className="px-2 font-bold text-[#1A1918]">{item.quantity}</span>
                      <button
                        onClick={() => onUpdateQuantity(item.product.id, item.selectedSize, item.quantity + 1)}
                        className="px-2 py-0.5 text-[#2C2A29] hover:bg-[#F5F0EB]"
                      >
                    ...
                    <span className="font-serif-display text-base font-bold text-[#1A1918]">
        ...
        {cartItems.length > 0 && (
          <div className="p-5 bg-white border-t border-[#EADFCF] space-y-3">
            
            {/* Promo Code Input */}
            <div>
              {appliedPromo ? (
                <div className="p-2 bg-[#EAE0D3] rounded border border-[#D5C5B5] flex items-center justify-between text-xs">
                  ...
                  <button 
                    ...
                    className="text-[10px] text-[#2C2A29] underline hover:text-[#C85A32]"
                  >
                    Remove
                  </button>
                </div>
                ...
                <form onSubmit={handleApplyPromo} className="flex gap-2">
                  <input
                    type="text"
                    placeholder="Enter Coupon Code (e.g. NAYAAB10)"
                    value={promoInput}
                    onChange={(e) => setPromoInput(e.target.value)}
                    className="flex-1 bg-[#FAF7F2] border border-[#EADFCF] rounded px-3 py-1.5 text-xs text-[#1A1918] focus:outline-none focus:border-[#C85A32]"
                  />
                  <button
                    type="submit"
                    className="px-4 py-1.5 bg-[#1E2B3A] text-white text-xs font-semibold rounded hover:bg-[#C85A32] transition"
                  >
                    Apply
                  </button>
                </form>
              )}
              {promoError && <p className="text-[10px] text-[#C85A32] mt-1">{promoError}</p>}
            </div>

            {/* Bill Breakdown */}
            <div className="space-y-1.5 text-xs text-[#7A6B5D] pt-2 border-t border-[#F5F0EB]">
              <div className="flex justify-between">
                <span>Bag Subtotal</span>
                <span className="font-semibold text-[#1A1918]">₹{subtotal.toLocaleString('en-IN')}</span>
              </div>
              {discountAmount > 0 && (
              ...
              <div className="flex justify-between text-base font-bold text-[#1A1918] pt-2 border-t border-[#EADFCF]">
                <span>Total Payable</span>
                <span className="font-serif-display text-xl text-[#C85A32]">
            ...
            <button
              ...
              className="w-full py-3.5 bg-[#C85A32] hover:bg-[#B04B26] text-white text-xs font-bold uppercase tracking-wider rounded-lg transition shadow-md flex items-center justify-center gap-2 group"
            >
